...
class Facade {
  ...
  void toFacade(PImage chainPhoto) {
	p.noSmooth();
    p.image(chainPhoto, 0, 0, p.width, p.height);
    for (int i = 0; i < numChainsTotal; i++) {
      for (int j = 0; j < chainLength; j++) { 
        chainDataArray[i][3*j + 0] = (byte)(p.red(chainPhoto.pixels[i*chainLength + j]));
        chainDataArray[i][3*j + 1] = (byte)(p.green(chainPhoto.pixels[i*chainLength + j]));
        chainDataArray[i][3*j + 2] = (byte)(p.blue(chainPhoto.pixels[i*chainLength + j]));
      }
    }
    for (int i = 0; i < numChainsTotal; i++) {
